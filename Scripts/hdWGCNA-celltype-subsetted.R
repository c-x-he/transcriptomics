...
setwd("~/new_frontal_official/hdWGCNA/Oli-subset")

seurat_obj<-BrainFRONTAL.noMT
seurat_obj<- subset(x = seurat_obj, subset = Cluster.id == "Oligodendrocytes")
...
seurat_obj <- SetDatExpr(
  seurat_obj,
  group_name = "Oligodendrocytes", # the name of the group of interest in the group.by column
...
PlotDendrogram(seurat_obj, main='Oligodendrocytes hdWGCNA Dendrogram')
...
seurat_obj <- ModuleConnectivity(
  seurat_obj,
  group.by = 'Cluster.id', group_name = 'Oligodendrocytes')
...
hME<-data.frame(diagnosis=seurat_obj@meta.data[["Diagnosis"]], hME=seurat_obj@meta.data[["Oli-M1"]])
...
seurat_obj <- readRDS("~/new_frontal_official/hdWGCNA/Oli-subset/hdWGCNA-seurat_obj-basic-end.rds")
